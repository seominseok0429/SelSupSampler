...
if __name__=="__main__":
    ...
    for epoch in range(opt.begin_epoch, opt.n_epochs + 1):
        ...
        for i, (inputs, targets) in enumerate(train_dataloader):
            ...
            mixed_index = np.random.randint(64, size=inputs.size()[0])
        
            targets = targets.cuda(non_blocking=True)
            inputs = Variable(inputs)
            inputs2 = Variable(inputs)
            

            for num, j in enumerate(rand_index):
                inputs[:,:,num,:,:] = inputs2[j, :, mixed_index[num], :,:]

            self_sup_targets = torch.tensor(mixed_index).cuda()

            targets = Variable(targets)
            outputs = model(inputs)
            loss = criterion(outputs, self_sup_targets)
